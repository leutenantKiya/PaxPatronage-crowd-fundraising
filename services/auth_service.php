<?php
    ...
    $user_type = (bool) $_POST['user_type']; 
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        // verif pass
        print_r($user_type != $user['user_type']);
        if ($user['user_type'] != $user_type){
            $tipe_user = $user_type == 0? "Penyelenggara":"Donatur";
            $msg = "Location: ../public/login.html?error=lu_bukan".$tipe_user;
            echo $msg;
            header($msg);
            exit;
        }
        if($user["pass"] == $password){
            ...
            $msg = "Location: ../public/".$page."?message=Selamat_Datang".$_SESSION['name'];
            header($msg);
            exit;
        }else{
            // wrong pass
